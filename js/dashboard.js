...
function updateDashboardUI(data, user) {
    ...
    // تحميل الكورسات المسجلة فعلياً من البيانات
...
function loadEnrolledCourses(enrolledCoursesData) {
    ...
    // تحويل الأوبجكت لمصفوفة عشان نقدر نلف عليها
    let myCourses = [];
    ...
    if (myCourses.length > 0) {
        list.innerHTML = myCourses.map(c => {
            ...
            // عرض التقييم لو موجود
            const ratingDisplay = c.userRating ? 
                `<span class="bg-yellow-100 text-yellow-700 text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1"><i data-lucide="star" class="w-3 h-3 fill-current"></i> ${c.userRating}</span>` : 
                '';

            // أزرار التحكم (مشاهدة / إتمام / شهادة)
            let actionButtons = '';
            
            if (isCompleted) {
                // لو خلص الكورس -> زرار الشهادة
                actionButtons = `
                ...
                // لو لسه -> زرار المشاهدة وزرار "تم الانتهاء"
                actionButtons = `
                    <a href="${c.url}" target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2 shadow-md">
                        <i data-lucide="play" class="w-4 h-4"></i> مشاهدة
                    </a>
                    <button onclick="markCourseComplete('${c.id}')" class="bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 px-4 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2">
                        <i data-lucide="check-circle" class="w-4 h-4"></i> خلصته
                    </button>
                `;
            }
...
// دالة تحديث حالة الكورس لـ "مكتمل"
window.markCourseComplete = function(courseId) {
    if (!currentFirebaseUser) return;
    if(confirm("هل أنت متأكد أنك أتممت مشاهدة الكورس بالكامل؟ 🤓")) {
        const db = firebase.database();
        db.ref('users/' + currentFirebaseUser.uid + '/enrolledCourses/' + courseId).update({
            status: 'completed',
            progress: 100,
            completedAt: new Date().toISOString()
        }).then(() => {
            alert("عاش يا بطل! 🎉 تقدر دلوقتي تحمل شهادتك.");
        });
    }
}
...
window.generateCertificate = function(courseName) {
    ...
    // صورة قالب الشهادة (لازم تكون موجودة في مجلد الصور)
    // لو مش موجودة، هيستخدم لون سادة
    ...
    img.onload = () => {
        // أبعاد الكانفاس نفس أبعاد الصورة
        canvas.width = img.width;
        canvas.height = img.height;

        // 1. رسم الخلفية (الشهادة)
        ctx.drawImage(img, 0, 0);

        // 2. إعدادات النص (الاسم)
        ctx.font = 'bold 60px "Cairo", sans-serif'; // حجم ونوع الخط
        ctx.fillStyle = '#1e293b'; // لون النص (رمادي غامق)
        ctx.textAlign = 'center';

        // 3. كتابة الاسم (تغيير الأرقام دي حسب تصميم شهادتك)
        // x = نص العرض، y = الارتفاع اللي الاسم هيتكتب فيه
        ctx.fillText(userName, canvas.width / 2, canvas.height / 2 + 50);

        // 4. كتابة اسم الكورس (اختياري)
        ctx.font = '40px "Cairo", sans-serif';
        ctx.fillStyle = '#059669'; // لون أخضر
        ctx.fillText(courseName, canvas.width / 2, canvas.height / 2 + 150);
    ...
    img.onerror = () => {
        alert("صورة قالب الشهادة مش موجودة! تأكد من وجود ملف images/ui/certificate-template.jpg");
        btn.innerHTML = originalText;
        btn.disabled = false;
    };
}