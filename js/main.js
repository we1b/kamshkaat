...
window.enrollInCourse = function(courseId, courseType) {
    ...
    if (!course) {
        alert("حصل خطأ في بيانات الكورس!");
        return;
    }
    ...
    enrollmentRef.once('value', (snapshot) => {
        if (snapshot.exists()) {
            alert("انت مشترك في الكورس ده بالفعل! كمل تعلم في لوحة التحكم 🚀");
            ...
            enrollmentRef.set({
                ...
                alert("مبروك! تم الاشتراك في الكورس بنجاح 🎉\nهتلاقيه دلوقتي في لوحة التحكم بتاعتك.");
                window.location.href = "dashboard.html";
            }).catch((error) => {
                console.error(error);
...
window.submitRating = function(courseId, ratingValue) {
    ...
    const db = firebase.database();
    // حفظ التقييم في سجل الكورس وسجل المستخدم (اختياري)
